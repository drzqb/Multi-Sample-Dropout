...
params_patience = 7
params_mode = "train0"
params_check = "modelfiles/tta_msdrop_y_" + str(params_drop_num_project)
...
class Project(Layer):
    ...
    def build(self, input_shape):
        ...
        self.project = Dense(1, kernel_initializer=TruncatedNormal(stddev=0.02))
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(params_drop_num_project):
            output = self.dropoutproject[i](inputs)
            if i == 0:
                logits = self.project(output)
            else:
                logits += self.project(output)

        # logits /= params_drop_num_project

        prediction = tf.cast(tf.squeeze(tf.greater(logits, 0.0), axis=-1), tf.int32)
...
def metrics(logits, label):
    ...
    loss = tf.reduce_mean(binary_crossentropy(labelexpand, logits, from_logits=True))

    predict = tf.cast(tf.greater(logits, 0.0), tf.int32)
